...
** Last update Fri Apr 29 22:50:32 2016 Ethan Kerdelhue
*/
...
int			edit_win_size_x(t_prog *prog)
{
  ...
  while (flag != 1)
	...
	my_printf(0, "Error : la valeur doit être comprise entre 1 et 1980\n");
      free(str);
    }
  return (0);
}
...
int			edit_win_size_y(t_prog *prog)
{
  ...
  while (flag != 1)
	...
	my_printf(0, "Error : la valeur doit être comprise entre 1 et 1080\n");
      free(str);
    }
  return (0);
}
...
{
  my_printf(0, "Position de la caméra\n");
  if ((edit_point(&prog->cam_pos)) == -1)
    return (-1);
  my_printf(0, "Regard de la caméra\n");
  if ((edit_point(&prog->cam_pos)) == -1)
    return (-1);
  return (0);
}
...
int			edit_cam(t_prog *prog)
{
  ...
  if (aff_win_prop(0, prog) == -1)
    return (-1);
  my_printf(0, "Laissez vide pour concerver la valeur\n");
  my_printf(0, "Screen size\n");
  if ((edit_win(prog)) == -1)
    return (-1);
  if ((edit_cam_pos(prog)) == -1)
    return (-1);
  my_printf(0, "Champs de vision (field of view)\n");
  if ((str = get_next_line(0)) == NULL)
    return (-1);
  if (str[0] != '\0')
    {
