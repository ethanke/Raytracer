...
** Last update Thu May  5 00:57:27 2016 Victor Sousa
*/
...
t_obj_list              *add_pill(t_obj_list *prev, char **file, int id)
{
  ...
  if ((p->cap[0] = malloc(sizeof(t_sphere))) == NULL)
    return (NULL);
  p->cap[0]->center = minus_vector(p->cylin->center, float_time_vector((float)p->cylin->height / 2.0, p->cylin->dir));
  ...
  if ((p->cap[1] = malloc(sizeof(t_sphere))) == NULL)
    return (NULL);
  p->cap[1]->center = add_vector(p->cylin->center, float_time_vector((float)p->cylin->height / 2.0, p->cylin->dir));
  p->cap[1]->material = p->cylin->material;
  p->cap[1]->material2 = p->cylin->material;
  p->cap[1]->radius = p->cylin->radius;
  p->cylin->center = minus_vector(p->cylin->center, float_time_vector((float)p->cylin->height / 2.0 , p->cylin->dir));
  new->obj = p;
  new->type = 'l';
  new->next = prev;
  return (new);
