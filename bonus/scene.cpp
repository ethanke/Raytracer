...
Scene::Scene(QWidget *parent)
{
    ...
    while (i <= this->objectCount)
    {
        ...
        if (this->myxml->get_field(s.toLatin1().data()) == "sphere")
            this->objectList.push_back(new Sphere(Vector3f(QString("scene:object_list:obj") + QString::number(i) + QString(":center:"), this->myxml),  this->myxml->get_field((QString("scene:object_list:obj") + QString::number(i) + ":radius").toLatin1().data()).toFloat(), new Material(1, Color(1.0, 0.1, 1.0), 50, 0, Image("bite"))));
